...
v = CastNoTarget(#神圣复仇者#, vars["团血危急"] and talent["神圣复仇者"] and not player.Buff:Has("复仇十字军") and (vars["40码平均血量"] < 0.35 or not player.Buff:Has("复仇之怒"))) if v then return v end
v = CastNoTarget(#光环掌握#, vars["团血危急"]) if v then return v end

if vars["团血危急"] and (vars["40码平均血量"] < 0.35 or not player.Buff:Has("神圣复仇者")) then
...
v = CastNoTarget(#美德道标#, vars["40码平均血量"] < 0.9 and talent["美德道标"]) if v then return v end
v = CastOnTarget(#殉道者之光#, targetHealthPercent < 0.4 and not UnitIsUnit("target", "player") and target:CanBeAssisted() and (playerHealthPercent > 0.4 or player.Buff:Has("圣盾术"))) if v then return v end
