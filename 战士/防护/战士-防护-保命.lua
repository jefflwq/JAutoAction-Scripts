...
vars["盾牌格挡中"] = player.Buff:Has("盾牌格挡")
...
UpdateReduce("player")
v = CastNoTarget(#盾牌格挡#, SwitchIsOn("盾牌格挡") and playerHealthPercent < 0.9 and vars["3秒物理伤害"] > 0.08 and #盾牌格挡#:GetChargeInfo():IsReachedOrSoon() and not vars["盾牌格挡中"] and (vars["当前减伤"] < 0.15 or playerHealthPercent < 0.2) and not (talent["激励"] and player.Buff:Has("破釜沉舟"))) if v then return v end
v = CastNoTarget(#挫志怒吼#, SwitchIsOn("挫志怒吼") and playerHealthPercent < 0.65 and vars["3秒物理伤害"] > 0.08 and not vars["盾牌格挡中"] and (vars["当前减伤"] < 0.15 or playerHealthPercent < 0.35)) if v then return v end
...
v = CastNoTarget(#盾牌格挡#, SwitchIsOn("盾牌格挡") and playerHealthPercent < 0.9 and (vars["3秒物理伤害"] > 0.12 or (vars["3秒物理伤害"] > 0.1 and playerHealthPercent < 0.5)) and not vars["盾牌格挡中"] and (vars["当前减伤"] < 0.15 or playerHealthPercent < 0.2) and not (talent["激励"] and player.Buff:Has("破釜沉舟"))) if v then return v end
